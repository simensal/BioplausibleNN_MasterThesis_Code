...
def main(argv):
    ...
    ### Finished PCN training. Should store the flops performed by PCN so far, and run the ANN until it reaches roughly the same number of flops. ###
    
    stopping_flops = np.mean(pcns_training_flops)
    ann_performed_epochs = 0
    # Training ANN agent
    print("\n ------------------ ANN Training ------------------ \n")
    anns_flop_counter = np.zeros(len(anns))

    print("Training agents...")
    while np.mean(anns_flop_counter) < stopping_flops and ann_performed_epochs < max_epochs:
        anns_training_flops = []
        for agent in anns: 
            anns_training_flops.append(train_agent(agent, X_train_bp, y_train, epochs, verbose, sample_interval)[1])
        anns_flop_counter += anns_training_flops
        ann_performed_epochs += epochs

    print(f'Average flops by agent over {ann_performed_epochs} epochs: {np.mean(anns_flop_counter):,}'.replace(',', ' '))
    print(f'Average flops per epoch: {np.mean(anns_flop_counter)/ann_performed_epochs:,}'.replace(',', ' '))
    ...
    if save:
        df = pd.DataFrame({
            ...
            'ann_tflops': anns_flop_counter,
            'ann_epochs': np.ones(len(anns))*ann_performed_epochs,
            'pcn_tflops': pcns_training_flops,
            'pcn_epochs': np.ones(len(pcns))*epochs,
        ...
        df.to_csv('./experiments/flops/results/'+ dataset_name +'/flop_stopping.csv', index=False)
